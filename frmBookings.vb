...
#Region "Declaration"
    ...
    Dim objCommissionDetailsBal As New clsCommissionDetailsBAL
    ...
    Dim commissionData As New List(Of clsCommissionDetails)
    Dim chosenRent As New clsRentDetails
    Dim chosenCommission As New clsCommissionDetails
...
TxtLFGrossAmt6.KeyPress, TxtLFLEEAgt6.KeyPress, TxtLFSplitPct6.KeyPress, TxtPFGrossAmt6.KeyPress, TxtPFLEEAgt6.KeyPress, TxtPFSplitPct6.KeyPress
        ...
        Try
            If GrdRentDetails.SelectedRows.Count > 0 Then
                ...
                If selectedRow.Cells("RentID").Value IsNot Nothing Then
                    ...
                    ' Synchronize selection with grdCommDetails
                    SyncGrdCommDetailsSelection(RentID)
    ...
    Private Sub grdCommDetails_SelectionChanged(sender As Object, e As EventArgs) Handles GrdCommDetails.SelectionChanged
        Try
            If GrdCommDetails.SelectedRows.Count > 0 Then
                Dim selectedRow As DataGridViewRow = GrdCommDetails.SelectedRows(0)
                If selectedRow.Cells("RentID").Value IsNot Nothing Then
                    RentID = CInt(selectedRow.Cells("RentID").Value)
                    Dim comm As clsCommissionDetails = objCommissionDetailsBal.GetByID(RentID)
                    If comm IsNot Nothing AndAlso comm.RentID <> 0 Then
                        SetCommissionFieldValues(comm)
                    End If
    ...
    Private Sub BtnCommSave_Click(sender As Object, e As EventArgs) Handles BtnCommSave.Click
        Try
            ' Populate the chosenCommission object with current form values
            PopulateCommissionFromFields()
            objCommissionDetailsBal.Update(chosenCommission)
                MessageBox.Show("Commission details updated successfully.", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information)
            'End If
            LoadAllCommData()
        Catch ex As Exception
            MessageBox.Show($"Error saving commission details: {ex.Message}", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
        End Try
    End Sub

    Private Sub TxtTCommListCallPct_Leave(sender As Object, e As EventArgs) Handles TxtCommListCalPct.Leave
        If Not (String.IsNullOrEmpty(TxtCommListCalPct.Text)) Then
            TxtCommListCalAmt.Text = (TxtCommTotRent.Text * TxtCommListCalPct.Text) / 100
        End If
    End Sub

    Private Sub TxtTCommProcCallPct_Leave(sender As Object, e As EventArgs) Handles TxtCommProcCalPct.Leave
        If Not (String.IsNullOrEmpty(TxtCommProcCalPct.Text)) Then
            TxtCommProcCalAmt.Text = (TxtCommTotRent.Text * TxtCommProcCalPct.Text) / 100
        End If
    End Sub
...
#Region "Methods"
    ...
    Private Sub PopulateCommissionFromFields()
        chosenCommission.TotalRentCalculated = Decimal.Parse(TxtCommTotRent.Text)
        chosenCommission.ListCommPct = Decimal.Parse(TxtCommListCalPct.Text)
        chosenCommission.ListCommAmtCalculated = Decimal.Parse(TxtCommListCalAmt.Text)
        chosenCommission.ProcCommPct = Decimal.Parse(TxtCommProcCalPct.Text)
        chosenCommission.ProcCommAmtCalculated = Decimal.Parse(TxtCommProcCalAmt.Text)
        chosenCommission.ListCommAmtOverride = Decimal.Parse(TxtCommListAmtOver.Text)
        chosenCommission.ProcCommAmtOverride = Decimal.Parse(TxtCommProcAmtOver.Text)

    End Sub

    Private Sub SyncGrdCommDetailsSelection(rentID As Integer)
        For Each row As DataGridViewRow In GrdCommDetails.SelectedRows
            row.Selected = False
        Next
        For Each row As DataGridViewRow In GrdCommDetails.Rows
            If row.Cells("RentID").Value IsNot Nothing AndAlso CInt(row.Cells("RentID").Value) = rentID Then
                row.Selected = True
                GrdCommDetails.FirstDisplayedScrollingRowIndex = row.Index
                Exit For
            End If
        Next
    End Sub
...
#End Region
    ...
    .TotalConsidation = If(Decimal.TryParse(TxtTotConsidation.Text, Nothing), Decimal.Parse(TxtTotConsidation.Text), 0),
        ...
        .ListTotalFeesManual = If(Decimal.TryParse(TxtListingManual.Text, Nothing), Decimal.Parse(TxtListingManual.Text), 0),
        ...
        .ProcTotalFeesManual = If(Decimal.TryParse(TxtProcuringManual.Text, Nothing), Decimal.Parse(TxtProcuringManual.Text), 0),
    ...
    .ListTotalFeesManual = If(Decimal.TryParse(TxtListingManual.Text, Nothing), Decimal.Parse(TxtListingManual.Text), 0),
        ...
        Catch ex As Exception
            ' Log the exception if necessary
            ' logger.Log(ex.ToString())
            MessageBox.Show("Failed to update the booking.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
        End Try
    End Sub
    ...
    Private Sub IniRentGrid()
        Try
            ...
            DisableSortingForGridView(GrdRentDetails)
            IniCommissionGrid()
        Catch ex As Exception
            Throw ex
        End Try
    ...
    Private Sub IniCommissionGrid()
        ...
        GrdCommDetails.Columns.Add("RentID", "Rent ID")
        ...
        DisableSortingForGridView(GrdCommDetails)
    End Sub
    ...
    Private Sub LoadAllCommData()
        commissionData = Nothing
        commissionData = objCommissionDetailsBal.GetByBookingID(BookingID)
    ...
    Private Sub ShowCommissionData(data As List(Of clsCommissionDetails))
        GrdCommDetails.Rows.Clear()
        If (data.Count = 0) Then
            GrdCommDetails.Rows.Clear()
        Else
            Dim totalListCommAmt As Decimal = 0
            Dim totalProcCommAmt As Decimal = 0
            For Each item As clsCommissionDetails In data
                Dim values As Object() = {
                item.RentID,
                item.TotalRentCalculated,
                item.ListCommPct,
                item.ListCommAmtCalculated,
                item.ListCommAmtOverride,
                item.ProcCommPct,
                item.ProcCommAmtCalculated,
                item.ProcCommAmtOverride
            }
                GrdCommDetails.Rows.Add(values)

                If item.ListCommAmtCalculated <> Nothing Then
                    totalListCommAmt += item.ListCommAmtCalculated
                End If

                If item.ProcCommAmtCalculated <> Nothing Then
                    totalProcCommAmt += item.ProcCommAmtCalculated
                End If
            Next

            If (totalListCommAmt <> 0) Then
                'TxtListCommSumTotal.Text = totalListCommAmt.ToString()
            End If

            If (totalProcCommAmt <> 0) Then
                'TxtProcCommSumTotal.Text = totalProcCommAmt.ToString()
            End If
        End If

        If GrdCommDetails.Rows.Count < 12 Then
            For Counter As Integer = (GrdCommDetails.Rows.Count + 1) To 12
                Dim values As Object() = {
                0,
                DBNull.Value,
                DBNull.Value,
    ...
    Private Sub SetCommissionFieldValues(commDetails As clsCommissionDetails)
        chosenCommission = commDetails
        TxtCommTotRent.Text = commDetails.TotalRentCalculated.ToString()
        TxtCommListCalPct.Text = commDetails.ListCommPct.ToString()
        TxtCommListCalAmt.Text = commDetails.ListCommAmtCalculated.ToString()
        TxtCommProcCalPct.Text = commDetails.ProcCommPct.ToString()
        TxtCommProcCalAmt.Text = commDetails.ProcCommAmtCalculated.ToString()
        TxtCommListAmtOver.Text = commDetails.ListCommAmtOverride.ToString()
        TxtCommProcAmtOver.Text = commDetails.ProcCommAmtOverride.ToString()
    End Sub

    Private Sub DisableSortingForGridView(grid As DataGridView)
...
#Region "Sale calculation"
    ...
    Private Sub CalculateTotalFee(commRateTextBox As TextBox, totalConsiderationTextBox As TextBox, manualFeeCheckBox As TextBox, totalFeeTextBox As TextBox)
        ...
        If Not String.IsNullOrEmpty(manualFeeCheckBox.Text) Then
            totalFeeTextBox.Text = manualFeeCheckBox.Text
